...
with open(conv_path, 'r') as f:
    ...
    for line in f:
        ...
        if line == '':
            continue
        if line[0] == 'E':
            if one_conv and one_conv[-1].find("=/。/=") == -1 and one_conv[-1].find("=/ /=") == -1:
                convs.append(one_conv)
            one_conv = []
        elif line[0] == 'M':
            one_conv.append(line.replace('M ', '', 1))
"""
...
def convert_seq2seq_files(questions, answers, TESTSET_SIZE=8000):
    # 创建文件
    train_enc = open('dataset3/train.enc', 'w')  # 问
    train_dec = open('dataset3/train.dec', 'w')  # 答
    test_enc = open('dataset3/test.enc', 'w')  # 问
    test_dec = open('dataset3/test.dec', 'w')  # 答
