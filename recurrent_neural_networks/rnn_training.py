...
train_encode_vec = 'dataset3/train_encode.vec'
train_decode_vec = 'dataset3/train_decode.vec'
test_encode_vec = 'dataset3/test_encode.vec'
test_decode_vec = 'dataset3/test_decode.vec'

# 词汇表大小5000
vocabulary_encode_size = 135890
vocabulary_decode_size = 137423

buckets = [(5, 10), (10, 15), (20, 25), (40, 50), (80, 120), (150, 200)]
layer_size = 256  # 每层大小
num_layers = 3  # 层数
batch_size = 128
...
with tf.Session(config=config) as sess:
    ...
    if ckpt != None:
        total_step = model.global_step.eval()
    ...
    while True:
        ...
        if total_step % 100 == 0:
            print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + ">>>>" + str(total_step) + "|loss:" + str(loss) + "|step_loss:" + str(step_loss))
        ...
        if total_step % 500 == 0:
            ...
            # 保存模型
            checkpoint_path = "model/chatbot_seq2seq.ckpt"
